...
export function SettingsNative({ profile, onUpdateProfile, onNavigateEquipment, onAddWeight, onBack }: SettingsProps) {
  ...
  return (
    <div class="flex-1 overflow-y-auto no-scrollbar pb-24">
      ...
      <div class="px-4 pt-4 space-y-6">
          ...
          <div class="bg-surface-dark rounded-xl p-4 border border-white/5">
            <div class="flex items-start gap-3">
              ...
              <div class="text-sm text-slate-400 space-y-1.5">
                <p>Your workout data and profile are stored securely in Supabase and synced across your devices. Data is protected by row-level security — only you can access your data.</p>
        ...
        {/* Account */}
        <div class="space-y-2">
          <h3 class="text-sm font-bold text-slate-400 uppercase tracking-wider px-1">Account</h3>
          <button
            onClick={async () => { await signOut(); window.location.reload(); }}
            class="w-full flex items-center justify-center gap-2 p-4 bg-surface-dark rounded-xl border border-red-500/20 hover:border-red-500/40 transition-colors"
          >
            <Icon name="logout" class="text-red-400" />
            <span class="font-semibold text-red-400 text-sm">Sign Out</span>
          </button>
        </div>
